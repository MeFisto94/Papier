...
namespace Papier
{
    class Program
    {
            ...
            Parser.Default.ParseArguments<ProgramOptions>(args).WithParsed(o =>
            {
                ...
                switch (o.Method.ToLower())
                {
                    ...
                    case "sln":
                    {
                        ...
                        assemblyList.AsParallel().ForAll(tuple =>
                        {
                            ...
                            settings.SortCustomAttributes = true; // We need consistent ordering.
                    ...
                    case "buildbsdiff":
                    {
                        ...
                        if (o.Method.ToLower().Equals("buildbsdiff"))
                        {
                            var outputPath = Path.Combine("work", "bin");
                            assemblyList.ForEach(tuple =>
                            {
                                var (assemblyFile, assembly) = tuple;
                                var moduleName = Path.GetFileNameWithoutExtension(assemblyFile)!;
                                
                                var baseArr = File.ReadAllBytes(Path.Combine(outputPath, $"{moduleName}-base.dll"));
                                var compiledArr = File.ReadAllBytes(Path.Combine(outputPath, $"{moduleName}-compiled.dll"));

                                using (var patchOutputStream = File.Open(Path.Combine(outputPath, $"{moduleName}-bsdiff.dll"),
            ...
            DefaultAssemblyResolver assemblyResolver)
        {
            ...
            var comp = CSharpCompilation.Create($"{moduleName}-diff.dll", sourceFiles, refs, 
